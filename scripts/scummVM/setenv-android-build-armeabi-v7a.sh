...
if [ "$VANILLA_PATH" == "" ]; then
	...
	export ANDROID_TARGET_NDK_TOOLCHPREFIX=arm-linux-androideabi
	export ANDROID_TARGET_NDK_LIBS_SUBPATH=armv7a
	...
	export CPPFLAGS=
	export CXX="ccache armv7a-linux-androideabi16-clang++"
	...
	echo "   make clean; ./configure --enable-all-engines --enable-verbose-build --host=android-arm-v7a --enable-debug"
	echo "   make clean; ./configure --disable-all-unstable-engines --enable-verbose-build --host=android-arm-v7a --enable-debug"
	echo "   make clean; ./configure --disable-all-engines --enable-engine=<engine B> --enable-engine=<engine A> --host=android-arm-v7a --enable-debug"
	echo "   make clean; ./configure --enable-verbose-build --host=android-arm-v7a --enable-release --disable-debug"
