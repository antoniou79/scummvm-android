...
if [ "$VANILLA_PATH" == "" ]; then
	...
	export ANDROID_TARGET_NDK_TOOLCHPREFIX=x86_64
	export ANDROID_TARGET_NDK_LIBS_SUBPATH=x86_64
	...
	echo "To build for Android x86_64..."
	echo "From ScummVM source folder you may run *one* of the following configure commands or your own custom configure:"
	echo "   make clean; ./configure --enable-all-engines --enable-verbose-build --host=android-x86_64 --enable-debug"
	echo "   make clean; ./configure --disable-all-unstable-engines --enable-verbose-build --host=android-x86_64 --enable-debug"
	echo "   make clean; ./configure --disable-all-engines --enable-engine=<engine B> --enable-engine=<engine A> --host=android-x86_64 --enable-debug"
	echo "   make clean; ./configure --enable-verbose-build --host=android-x86_64 --enable-release --disable-debug"
